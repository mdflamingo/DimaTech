...
class UserRepository:
    ...
    async def create(self, user: UserCreate, session: AsyncSession) -> None:
        try:
            new_user = User(
                email=user.email, password=user.password, full_name=user.full_name
            )
    ...
    async def update(self, email: str, user: UserUpdate, session: AsyncSession) -> None:
        try:
            ...
            if "password" in update_values and update_values["password"]:
                update_values["password"] = generate_password_hash(
            ...
            if result.rowcount == 0:
                raise ValueError(
                    f"Not found {self.db_model.__name__} with email: {email}"
                )
        except Exception as exc:
            await session.rollback()
            raise exc
    ...
    async def get_user_list(self, session: AsyncSession) -> list[UserList]:
        try:
            query = select(self.db_model).options(selectinload(self.db_model.accounts))
            result = await session.execute(query)
        ...
        return [
            UserList(
                ...
                accounts=[
                    Account(balance=account.balance) for account in user.accounts
                ],
            )
            for user in users
        ]
